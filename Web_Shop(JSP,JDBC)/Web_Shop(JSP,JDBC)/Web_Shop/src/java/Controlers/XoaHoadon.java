 ...
 * @author manht
 */
@WebServlet(name = "XoaHoadon", urlPatterns = {"/XoaHoadon"})
public class XoaHoadon extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int mahd=0;
            String url="index_admin.jsp?module=hoadon&cmd=xoa";
            ...
            if(sMaHD==null || sMaHD.equals("") || Dungchung.isInteger(sMaHD)==false)
                url  += "&ketqua=Loi_MaHD";
                 ...
                 tbHoadon bangHoadon = new tbHoadon();
                  int ketqua = bangCTHD.XoaChitietHoadon(mahd);
                 if(ketqua!=1)
                     url += "&ketqua=Loi_XoaCTHD";
                  else{
                     ketqua = bangHoadon.XoaHoadon(mahd);
                     if(ketqua!=1)
                        url += "&ketqua=Loi_XoaHD";
                     else{
                         url = "index_admin.jsp?module=hoadon&cmd=xoa&ketqua=OK";
                     }
                 }
            }
