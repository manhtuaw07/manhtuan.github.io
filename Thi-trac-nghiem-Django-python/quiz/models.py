...
class Category(models.Model):
    ...
    class Meta:
        verbose_name = _("Chuyên Ngành")
        verbose_name_plural = _("Chuyên Ngành")
...
class Quiz(models.Model):
    ...
    title = models.CharField(
        verbose_name=_("Tên môn"),
        max_length=60, blank=False)

    description = models.TextField(
        verbose_name=_("Mô tả"),
        blank=True, help_text=_("Mô tả"))

    url = models.SlugField(
        max_length=60, blank=False,
        help_text=_("Chữ hoặc số bất kỳ"),
        verbose_name=_("user friendly url"))

    category = models.ForeignKey(
        Category, null=True, blank=True,
        verbose_name=_("Chuyên ngành"), on_delete=models.CASCADE)

    random_order = models.BooleanField(
        blank=False, default=False,
        verbose_name=_("Random Order"),
        help_text=_("Đảo câu hỏi"))

    max_questions = models.PositiveIntegerField(
        blank=True, null=True, verbose_name=_("Max Questions"),
        help_text=_("Số câu trong 1 trang"))

    answers_at_end = models.BooleanField(
        blank=False, default=False,
        help_text=_("Đáp án sẽ hiện sau khi bài làm hoàn thành"),
        verbose_name=_("Answers at end"))

    exam_paper = models.BooleanField(
        blank=False, default=False,
        help_text=_("Kết quả sẽ hiện ngay khi người dùng trả lời"),
        verbose_name=_("Exam Paper"))

    single_attempt = models.BooleanField(
        blank=False, default=False,
        help_text=_("Chỉ cho phép người dùng làm bài 1 lần duy nhất"),
        verbose_name=_("Single Attempt"))

    pass_mark = models.SmallIntegerField(
        blank=True, default=0,
        verbose_name=_("Pass Mark"),
        help_text=_("Tỷ lệ % cần thiết để vượt qua kỳ thi"),
        validators=[MaxValueValidator(100)])

    success_text = models.TextField(
        blank=True, help_text=_("Hiển thị sau khi người dùng vượt qua bài thi"),
        verbose_name=_("Success Text"))

    fail_text = models.TextField(
        verbose_name=_("Fail Text"),
        blank=True, help_text=_("Hiển thị khi người dùng trượt"))

    draft = models.BooleanField(
        blank=True, default=False,
        verbose_name=_("Draft"),
        help_text=_("Bản nháp"))
    ...
    class Meta:
        verbose_name = _("Môn Thi")
        verbose_name_plural = _("Môn Thi")
    ...
    def list_all_cat_scores(self):
        ...
        for cat in Category.objects.all():
            ...
            if match:
                ...
                try:
                    percent = float(round(((float(score) / float(possible))* 10), 1))
                except:
                    percent = 0
        ...
        if match:
            ...
            new_score = ",".join(
                [
                    ...
                    # str(float(round(((float(updated_score) / float(updated_possible))* 10), 1))),
                    " "
                ])
            ...
            self.score = ",".join(
                [
                    ...
                    str(score_to_add),
                    str(possible_to_add),
                    # str(float(round(((float(score_to_add) / float(possible_to_add)) * 10), 1))),
                    ""
                ])
            self.save()
...
class Question(models.Model):
    ...
    explanation = models.TextField(max_length=2000,
                                   blank=True,
                                   # help_text=_("Giải Thích"),
                                   verbose_name=_('Giải Thích'))
